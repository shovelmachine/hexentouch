...
#define amscreen subscreen
#define AMSCREENWIDTH DRAWSCREENWIDTH
#define AMSCREENHEIGHT DRAWSCREENHEIGHT
#else
#define amscreen screen
#define AMSCREENWIDTH SCREENWIDTH
#define AMSCREENHEIGHT SCREENHEIGHT
#endif
...
static int finit_width = AMSCREENWIDTH;
static int finit_height = AMSCREENHEIGHT-SBARHEIGHT-3;
...
//byte screens[][AMSCREENWIDTH*AMSCREENHEIGHT];
...
void AM_loadPics(void)
{
  ...
  maplump = W_CacheLumpName("AUTOPAGE", PU_CACHE);
  //rww end
}
...
//			dmapx=0;  //goes into the automap.
	...
	for(i = 0; i < AMSCREENHEIGHT-SBARHEIGHT; i++)
	{
		memcpy(amscreen+i*finit_width, maplump+j+mapxstart, 	
			finit_width-mapxstart);
		memcpy(amscreen+i*finit_width+finit_width-mapxstart, maplump+j, 
			mapxstart);
		j += finit_width;
...
//	 memcpy(amscreen, maplump, finit_width*finit_height);
...
//	if(gameskill == sk_baby) AM_drawkeys();
	...
	MN_DrTextSecondaryA(P_GetMapName(gamemap), 38, 144);
...
void AM_DrawDeathmatchStats(void)
{
	...
	for(i = 0; i < MAXPLAYERS; i++)
	{
		...
		{
			MN_DrTextSecondaryA(PlayerColorText[order[i]], 8, yPosition);
			sprintf(textBuffer, "%d", fragCount[order[i]]);
			MN_DrTextSecondaryA(textBuffer, 80, yPosition);
			yPosition += 10;
		}
	}
}
...
static void DrawWorldTimer(void)
{
	...
	MN_DrTextSecondaryA(timeBuffer, 240, 8);
	...
	if (days)
	{
		...
		MN_DrTextSecondaryA(dayBuffer, 240, 20);
		if (days >= 5)
		{
			MN_DrTextSecondaryA("YOU FREAK!!!", 230, 35);
		}
	}
}
