...
//rww begin
static void SCGamma(int option);
static void SCInvLook(int option);
static void SCSkies(int option);
static void SCGiveAll(int option);
static void SCWarpLevel(int option);
...
static MenuItem_t MainItems[] =
{
	...
	{ ITT_SETMENU, "EXTRA", NULL, 0, MENU_DS },
	//rww end
};
...
//	{ ITT_EMPTY, NULL, NULL, 0, MENU_NONE },
	{ ITT_EFUNC, "TOUCH INVERT : ", SCInvLook, 0, MENU_NONE },
	//rww begin
	//{ ITT_LRFUNC, "MOUSE SENSITIVITY", SCMouseSensi, 0, MENU_NONE },
	{ ITT_LRFUNC, "TOUCH SENSITIVITY", SCMouseSensi, 0, MENU_NONE },
	//rww end
	{ ITT_EMPTY, NULL, NULL, 0, MENU_NONE },
...
//rww begin
static MenuItem_t DSItems[] =
{
	{ ITT_EFUNC, "HIGHDETAIL : ", SCSkies, 0, MENU_NONE },
...
static MenuItem_t WarpItems[] =
{
	{ ITT_LRFUNC, "LEVEL", SCWarpLevel, 0, MENU_NONE },
	{ ITT_EMPTY, NULL, NULL, 0, MENU_NONE },
	{ ITT_EFUNC, "GO", SCWarpGo, 0, MENU_NONE },
};

static Menu_t WarpMenu =
{
	88, 30,
	DrawWarpMenu,
	3, WarpItems,
...
//rww begin
void MN_DrTextSecondaryA(char *text, int x, int y)
{
	char c;
	patch_t *p;
...
#ifndef _HEXENDS //rww begin FIXME
		fp = fopen(name, "rb");
		if(fp)
		{
...
static void DrawWarpMenu(void)
{
	...
	DrawSlider(&WarpMenu, 1, 13, warpToLevel-1);

	sprintf(str, "%i", warpToLevel);
	MN_DrTextB(str, 196, 30);
}
//rww end
...
extern int invertLook; //rww begin rww end
static void DrawOptionsMenu(void)
{
	//rww begin
	if(invertLook)
	{
		MN_DrTextB("ON", 222, 50);
	}
	else
	{
		MN_DrTextB("OFF", 222, 50);
	}
	//rww end
...
{
	//rww begin
	if(messageson) //moved to options 2
	{
		MN_DrTextB("ON", 196, 100);
	}
	else
	{
		MN_DrTextB("OFF", 196, 100);
	}
	/*
#if defined( _DS_RESOLUTION ) || defined( _IPHONE )
	DrawSlider(&Options2Menu, 1, 6, screenblocks-3);
#else
	...
	*/
	DrawSlider(&Options2Menu, 1, 16, snd_MaxVolume);
	DrawSlider(&Options2Menu, 3, 16, snd_MusicVolume);
	//rww end
}
...
static void SCSkies(int option)
{
	...
	{
		detailLevel = 0;
		P_SetMessage(&players[consoleplayer], "DETAIL HIGH", true);
	}
	else
	{
		detailLevel = 1;
		P_SetMessage(&players[consoleplayer], "DETAIL LOW", true);
	}
...
void MN_DrawInfo(void)
{
	...
	//rww begin
#if defined( _DS_RESOLUTION ) || defined( _IPHONE )
	memcpy(subscreen, (byte *)W_CacheLumpNum(W_GetNumForName("TITLE")+InfoType,
		PU_CACHE), DRAWSCREENWIDTH*DRAWSCREENHEIGHT);
#else
