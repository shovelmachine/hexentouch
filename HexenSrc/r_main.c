...
void		(*altfuzzcolfunc) (void); //rww begin rww end
void		(*transfuzzcolfunc) (void); //rww begin rww end
...
int SlopeDiv (unsigned num, unsigned den)
{
	...
	if (den < 512)
		return SLOPERANGE;
	ans = HexenDivInt((num<<3), (den>>8)); //rww begin rww end
...
//
	...
	, finetangent[3072] );//, finetangent[FINEANGLES/4+FIELDOFVIEW/2] ); //rww begin rww end

	for (i=0 ; i</*FINEANGLES/2*/4096 ; i++) //rww begin rww end
	{
...
//
	...
	{
		startmap = ((LIGHTLEVELS-1-i)*2)*2;//NUMCOLORMAPS/LIGHTLEVELS; //rww begin rww end
		for (j=0 ; j<MAXLIGHTZ ; j++)
		{
			scale = FixedDiv ((SCREENWIDTH/2*FRACUNIT), (j+1)<<LIGHTZSHIFT);
			scale >>= LIGHTSCALESHIFT;
			level = startmap - HexenDivInt(scale, DISTMAP);
			if (level < 0)
				level = 0;
...
void R_ExecuteSetViewSize (void)
{
...
#if defined( _DS_RESOLUTION ) || defined( _IPHONE )
	if (setblocks == 8)
#else
	if (setblocks == 11)
#endif //rww end
	...
	else
	{
		...
		viewheight = (HexenDivInt(setblocks*161, 10)); //rww begin rww end
	}

	detailshift = setdetail;
	viewwidth = scaledviewwidth>>detailshift;

	centery = HexenDivInt(viewheight, 2); //rww begin rww end
	centerx = HexenDivInt(viewwidth, 2); //rww begin rww end
	...
	if (!detailshift)
	{
		...
		altfuzzcolfunc = R_DrawAltFuzzColumn;
		transfuzzcolfunc = R_DrawTranslatedFuzzColumn; //rww begin rww end
	...
	else
	{
		...
		fuzzcolfunc = R_DrawFuzzColumnLow; //rww begin rww end
		altfuzzcolfunc = R_DrawAltFuzzColumnLow; //rww begin rww end
		transfuzzcolfunc = R_DrawTranslatedFuzzColumnLow; //rww begin rww end
		transcolfunc = R_DrawTranslatedColumnLow; //rww begin rww end
...
// psprite scales
//
	pspritescale = HexenDivInt(FRACUNIT*viewwidth, SCREENWIDTH); //rww begin rww end
	pspriteiscale = HexenDivInt(FRACUNIT*SCREENWIDTH, viewwidth); //rww begin rww end
...
//
	...
	{
		dy = ((i-HexenDivInt(viewheight, 2))<<FRACBITS)+FRACUNIT/2; //rww begin rww end
		dy = abs(dy);
		yslope[i] = FixedDiv ( HexenDivInt((viewwidth<<detailshift), 2)*FRACUNIT, dy); //rww begin rww end
	}
...
//
	for (i=0 ; i< LIGHTLEVELS ; i++)
	{
		...
		{
			level = startmap - HexenDivInt(j*SCREENWIDTH, HexenDivInt((viewwidth<<detailshift), DISTMAP)); //rww begin rww end
			if (level < 0)
				level = 0;
			if (level >= NUMCOLORMAPS)
				level = NUMCOLORMAPS-1;
...
void R_SetupFrame(player_t *player)
{
	...
	tempCentery = HexenDivInt(viewheight, 2)+HexenDivInt((player->lookdir)*screenblocks, 10);
	if(centery != tempCentery)
	{
		...
		for(i = 0; i < viewheight; i++)
		{
			...
			yslope[i] = FixedDiv ((viewwidth<<detailshift)/2*FRACUNIT,
				...
				*/
			//rww begin rww end
			yslope[i] = FixedDiv (HexenDivInt((viewwidth<<detailshift), 2)*FRACUNIT,
				abs(((i-centery)<<FRACBITS)+FRACUNIT/2));
		}
	}
