...
#ifdef _IPHONE
extern void TouchScreenSample();
extern int TouchScreenLook();
extern int TouchScreenYaw();
#endif

// AREIS END
int invertLook = 0;
#define MAX_TOUCH_DIF	10 //15
#define MIN_TOUCH_DIF	2
extern unsigned int frameKeysHeld;
#ifdef _HEXENDS
void TouchScreenSample(void)
{
	touchPosition touch; //use newer libnds functionality

	if (!(frameKeysHeld & KEY_TOUCH))
	{
		curTouchXDif = 0;
		curTouchYDif = 0;
		lastTouchXDif = 0;
	...
	if (lastTouchX == -2 || lastTouchY == -2)
	{ //ignore the first touch frame for good measure
		lastTouchX = -1;
		lastTouchY = -1;
		return;
	}
	...
	curTouchXDif = touch.px;
	curTouchYDif = touch.py;
	if (lastTouchX == -1)
	{
		lastTouchX = curTouchXDif;
	}
	if (lastTouchY == -1)
	{
		lastTouchY = curTouchYDif;
	}

	curTouchXDif = lastTouchX-curTouchXDif;
	curTouchYDif = lastTouchY-curTouchYDif;
	if (abs(curTouchXDif) < MIN_TOUCH_DIF)
	{
		curTouchXDif = 0;
	}
	if (abs(curTouchYDif) < MIN_TOUCH_DIF)
	{
		curTouchYDif = 0;
	}

	if (curTouchXDif > MAX_TOUCH_DIF)
	{
		curTouchXDif = MAX_TOUCH_DIF;
	}
	else if (curTouchXDif < -MAX_TOUCH_DIF)
	{
		curTouchXDif = -MAX_TOUCH_DIF;
	}
	if (curTouchYDif > MAX_TOUCH_DIF)
	{
		curTouchYDif = MAX_TOUCH_DIF;
	}
	else if (curTouchYDif < -MAX_TOUCH_DIF)
	{
		curTouchYDif = -MAX_TOUCH_DIF;
	}
...
void G_BuildTiccmd (ticcmd_t *cmd)
{
	...
	*/
	...
	//rww begin
	if(gamekeydown[key_weaponcycle])
	{
		int curWeapon;
		player_t *pl;
	...
	//rww begin
	look += TouchScreenLook();
	//rww end

	//rww begin - line formatting confusing debugger
...
void G_DoLoadLevel (void)
{
	...
	// BEGIN AREIS
	// Start the automap up... automatically.
#ifdef _IPHONE
