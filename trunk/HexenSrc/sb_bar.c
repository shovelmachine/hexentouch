...
void SB_Drawer(void)
{
#if 0//#ifdef _DS_RESOLUTION //rww begin
	...
	//rww begin
	if(0)//viewheight == SCREENHEIGHT && !automapactive)
	//rww end
	{
		DrawFullScreenStuff();
		SB_state = -1;
...
#if 1
		//rww begin - draw to subscreen
		for(i = 165; i < 187-(22*CPlayer->mana[0])/MAX_MANA; i++)
		{
			subscreen[i*DRAWSCREENWIDTH+95] = 0;
			subscreen[i*DRAWSCREENWIDTH+96] = 0;
			subscreen[i*DRAWSCREENWIDTH+97] = 0;
		}
		V_DrawPatch(102, 164, manaVialPatch2);
		for(i = 165; i < 187-(22*CPlayer->mana[1])/MAX_MANA; i++)
		{
			subscreen[i*DRAWSCREENWIDTH+103] = 0;
			subscreen[i*DRAWSCREENWIDTH+104] = 0;
			subscreen[i*DRAWSCREENWIDTH+105] = 0;
		}
#endif
		//rww end
...
void Draw_TeleportIcon(void)
{
	...
	V_DrawPatchPrimary(68, 48, patch);
	//rww end
	UpdateState |= I_FULLSCRN;
	I_Update();
	UpdateState |= I_FULLSCRN;
